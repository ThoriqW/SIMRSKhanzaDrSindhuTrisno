...
public final class RMMonitoringBromagePascaAnestesi extends javax.swing.JDialog {
    ...
    private void MnDigitalTTEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDigitalTTEActionPerformed
        ...
        if(tbObat.getSelectedRow()>-1){
            ...
            if(Sequel.cariInteger("select count(no_rawat) from berkas_tte where no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"' and kode='022'") > 0){
                berkas.tampilPdf(FileName,"berkastte/laporan_anastesi_spinal",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString(),"022");
            }else{
                createPdf(FileName);
                berkas.tampilPdfLocal(FileName,"local","berkastte/laporan_anastesi_spinal",tbObat.getValueAt(tbObat.getSelectedRow(),0).toString(),"022");
            }
    ...
    */
    ...
    void createPdf(String FileName) {
       if(tbObat.getSelectedRow()>-1){
            ...
            Valid.MyReportPDFWithName1("rptMonitoringBromageScoreTTE.jasper","report","tempfile",FileName,"::[ Monitoring Bromage Score Pasca Anestesi ]::",
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.jk,pasien.tgl_lahir,skor_bromage_pasca_anestesi.tanggal,"+
                    "skor_bromage_pasca_anestesi.penilaian_skala1,skor_bromage_pasca_anestesi.penilaian_nilai1,skor_bromage_pasca_anestesi.keluar,"+
                    "skor_bromage_pasca_anestesi.instruksi,skor_bromage_pasca_anestesi.kd_dokter,dokter.nm_dokter,skor_bromage_pasca_anestesi.nip,petugas.nama "+
                    "from skor_bromage_pasca_anestesi inner join reg_periksa on skor_bromage_pasca_anestesi.no_rawat=reg_periksa.no_rawat "+
