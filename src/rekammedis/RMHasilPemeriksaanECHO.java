...
public final class RMHasilPemeriksaanECHO extends javax.swing.JDialog {
    ...
    public RMHasilPemeriksaanECHO(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            "No.Rawat","No.RM","Nama Pasien","Tgl.Lahir","Kode Dokter","Nama Dokter","Tanggal","Diagnosa Klinis","Kesimpulan"
        }){
    ...
    private void initComponents() {
        ...
        DiagnosaKlinis.setBounds(100, 80, 380, 23);
    ...
    */
        ...
        try{
            if(TCari.getText().trim().equals("")){
                ps=koneksi.prepareStatement(
                        ...
                        "hasil_pemeriksaan_echo.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_echo.diagnosa_klinis,hasil_pemeriksaan_echo.kesimpulan "+
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("diagnosa_klinis"),rs.getString("kesimpulan")
                    });
                }
            } catch (Exception e) {
    ...
    private void getData() {
        if(tbObat.getSelectedRow()!= -1){
            ...
            Kesimpulan.setText(tbObat.getValueAt(tbObat.getSelectedRow(),8).toString());
    ...
    private void ganti() {
        if(Sequel.mengedittf("hasil_pemeriksaan_echo","no_rawat=?","no_rawat=?,tanggal=?,kd_dokter=?,diagnosa_klinis=?,kesimpulan=?",6,new String[]{
                TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+"")+" "+Tanggal.getSelectedItem().toString().substring(11,19),KdDokter.getText(),
                DiagnosaKlinis.getText(),Kesimpulan.getText(),tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()
            })==true){
    ...
    private void simpan() {
        if(Sequel.menyimpantf("hasil_pemeriksaan_echo","?,?,?,?,?","No.Rawat",5,new String[]{
                TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+"")+" "+Tanggal.getSelectedItem().toString().substring(11,19),KdDokter.getText(),
                DiagnosaKlinis.getText(),Kesimpulan.getText()
            })==true){
                tabMode.addRow(new String[]{
    ...
    void createPdf(String FileName) {
       if(tbObat.getSelectedRow()>-1){
            ...
            Valid.MyReportPDFWithName1("rptCetakHasilPemeriksaanECHOTTE.jasper","report","tempfile",FileName,"::[ Formulir Hasil Pemeriksaan EKG ]::",
                "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,hasil_pemeriksaan_echo.tanggal,"+
                "hasil_pemeriksaan_echo.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_echo.diagnosa_klinis,"+
