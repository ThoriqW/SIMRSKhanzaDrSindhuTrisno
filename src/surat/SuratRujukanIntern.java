...
//        MnSurat.setVisible(false);
    ...
    private void initComponents() {
        ...
        TanggalSurat.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "30-08-2024 11:13:33" }));
        ...
        TanggalPeriksa.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "30-08-2024 11:13:33" }));
        ...
        if(TNoRM.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            if(Sequel.cariInteger("select no_surat from rujukan_internal_poli_report where no_surat='" + NoRawat.getText() + "'") > 0){
                ...
                Sequel.menyimpan("rujukan_internal_poli_report","?,?,?,?,?,?,?,?,?,?,?,?",12,new String[]{
                    ...
                    TanggalSurat.getSelectedItem().toString().substring(0, 10),
                    NmPoli.getText()
                });
                emptTeks();
  ...
  isForm();                
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if(TNoRM.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            if(tbObat.getSelectedRow()!= -1){
                Sequel.mengedittf("rujukan_internal_poli_report","no_surat=?","no_surat=? and no_rm=?,nama=?,jenis_kelamin=?,no_kartu_bpjs=?,alamat=?,diagnosa=?,dokter_pemeriksa=?,dokter_tujuan=?,tanggal_rujuk=?,tanggal_surat=?,poli_tujuan=?",13,new String[]{
                    NoRawat.getText(),
                    NoRawat.getText(),
                    TNoRM.getText(),
                    TPasien.getText(),
    ...
    private void MnDigitalTTEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDigitalTTEActionPerformed
        ...
        if(tbObat.getSelectedRow()>-1){
            ...
            if(Sequel.cariInteger("select count(no_rawat) from berkas_tte where no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"' and kode='030'") > 0){
        ...
        try {
            ps=koneksi.prepareStatement(
                    "select * from rujukan_internal_poli_report where "
                            ...
                            + "tanggal_surat like ? "
                            + "and tanggal_surat between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"'");
            try {
